...
export default function LiveMapSimulator({ isDeviated, speed = 42, eta = '12 phút' }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', minHeight: '340px', background: '#080d1e', borderRadius: '14px', overflow: 'hidden', border: '1px solid var(--border-card)' }}>
      {/* Map Header Overlay */}
      <div style={{ position: 'absolute', top: 12, left: 12, zIndex: 10, display: 'flex', gap: '8px', alignItems: 'center' }}>
        <div className="badge-ai" style={{ background: 'rgba(11, 19, 43, 0.85)', backdropFilter: 'blur(8px)', padding: '6px 12px' }}>
      ...
      <div style={{ position: 'absolute', bottom: 12, right: 12, zIndex: 10, background: 'rgba(15, 23, 42, 0.9)', backdropFilter: 'blur(8px)', border: '1px solid var(--border-card)', padding: '10px 14px', borderRadius: '10px', display: 'flex', gap: '16px', fontSize: '0.75rem' }}>
        <div>
        ...
        <div>
          ...
          <span style={{ color: '#34d399', fontWeight: 700, marginLeft: '6px', fontFamily: 'var(--font-mono)' }}>{eta}</span>
        </div>
        <div>
          <span style={{ color: 'var(--text-muted)' }}>Sai số Haversine:</span>
          <span style={{ color: isDeviated ? '#f87171' : '#a7f3d0', fontWeight: 700, marginLeft: '6px', fontFamily: 'var(--font-mono)' }}>
      ...
      <svg viewBox="0 0 800 400" style={{ width: '100%', height: '100%', display: 'block' }}>
        ...
        <path d="M 0 100 Q 200 120 400 90 T 800 130" fill="none" stroke="rgba(58, 80, 107, 0.25)" strokeWidth="4" />
        <path d="M 120 0 Q 150 200 200 400" fill="none" stroke="rgba(58, 80, 107, 0.25)" strokeWidth="4" />
        <path d="M 500 0 L 520 400" fill="none" stroke="rgba(58, 80, 107, 0.25)" strokeWidth="4" />
        ...
        {waypoints.map((wp) => (
          <g key={wp.id} transform={`translate(${wp.x}, ${wp.y})`}>
            ...
            {/* Label */}
            <rect x="-60" y="-38" width="120" height="22" rx="4" fill="rgba(15, 23, 42, 0.85)" stroke="rgba(58, 80, 107, 0.5)" strokeWidth="1" />
            <text x="0" y="-23" textAnchor="middle" fill="#f8fafc" fontSize="10" fontWeight="600">
              {wp.name}
            </text>
          </g>
        ))}
        ...
        <g transform={`translate(${busX}, ${busY})`} filter="url(#glow-bus)">
          ...
          {/* Bus Icon Container */}
          <rect x="-18" y="-14" width="36" height="28" rx="6" fill={isDeviated ? "#ef4444" : "#f59e0b"} stroke="#ffffff" strokeWidth="2" />
          <text x="0" y="4" textAnchor="middle" fill="#000" fontSize="11" fontWeight="800">
            BUS 01
          </text>
        </g>
      </svg>
