...
export default function CameraAiOverlay({ 
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', minHeight: '280px', background: '#050a17', borderRadius: '14px', overflow: 'hidden', border: isDrowsy ? '2px solid #ef4444' : '1px solid var(--border-card)' }}>
      ...
      {/* Video or Synthetic Feed Container */}
      {useWebcam ? (
        <video 
          ref={videoRef} 
          autoPlay 
        ...
        /* Synthetic Interactive AI Feed Simulator */
        <div style={{
          width: '100%',
          height: '100%',
          minHeight: '280px',
          ...
          {/* Mode 1: Driver Fatigue AI Bounding Overlay */}
          {mode === 'driver' && (
          ...
          {/* Mode 2: Student Face Recognition Scanner Overlay */}
          {mode === 'attendance' && (
            <div style={{ position: 'relative', width: '220px', height: '220px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{
                position: 'absolute',
      ...
      {webcamError && (
        <div style={{ position: 'absolute', bottom: 10, left: 10, right: 10, background: 'rgba(239, 68, 68, 0.9)', color: '#fff', padding: '6px 10px', borderRadius: '6px', fontSize: '0.7rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
